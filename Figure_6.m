% MATLAB code for generating Figure 6 of the manuscript
clc
clear
close all
%
...
mu = 2; k = 5; epsi = 1.5;
sig1 = 0.01; sig2 = 0.01; sig3 = 0.01;
qzero = [0; 0.6; 0.8]; % initial condition
wzero = [0.5; -0.9; 0]; % initial condition
%
...
wmean(:,1) = wzero;
%
% at all other time steps
for l=2:numel(t)
%
dB1_w(l) = sqrt(dt)*randn;
dB2_w(l) = sqrt(dt)*randn;
dB3_w(l)=sqrt(dt)*randn;
%
% MSIs
Iw_w=dB1_w(l); 
Iws_w=0.5*dt*(dB2_w(l)+dB3_w(l)/sqrt(3));
Isw_w=0.5*dt*(dB2_w(l)-dB3_w(l)); 
Iww_w=0.5*(dB1_w(l)*dB1_w(l)-dt); 
Iss_w = 0.5*(dt)^2;
...
%
 % g-Weak 3.0 solution
w(:,l) = w(:,l-1) + a*dt + sigmaw*dB1_w(l) + L0a*Iss_w +L1b*Iww_w + L1a*Iws_w...
...
%
for ii = 1:length(Q)
            Y_GW3(:,ii) = karcher_mean_sphere(Q(:,:,ii));
end
%
% Plotting
createfigure_fig6(Y_GW3(1,:),Y_GW3(2,:),Y_GW3(3,:))
%
% End